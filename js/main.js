...
class ESPNextGenIDE {
        ...
        try {
            ...
            components.forEach(component => {
                ...
                item.innerHTML = `
                    ...
                    <div class="meta">
                        <div class="title">${this.escapeHTML(component.name)}</div>
                        <div class="subtitle">${this.escapeHTML(component.category)}</div>
                    </div>
                `;

                item.addEventListener("dragstart", event => {
                    event.dataTransfer.setData("component", component.id);
                });
    ...
    run() {
        ...
        setTimeout(() => {
            ...
            const sent = this.network.sendJSON({
                type: "run",
                code: this.editor.getValue()
            });

            this.terminal[sent ? "success" : "warning"](
                sent ? "Code sent to ESP32" : "ESP32 not connected; running local simulation"
            );
